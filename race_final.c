...
int move_temp = 0;
const int cell_size_cm = 40;
const int cell_size_ticks = DPT * 40;
 ...
 */
void adjust_angle_triangle(int x1, int x2, char adj_dir) {
    ...
    if(direction == 'n' && adj_dir == 'e') radians = -radians;
    if(direction == 'e' && adj_dir == 's') radians = -radians;
    if(direction == 's' && adj_dir == 'w') radians = -radians;
    if(direction == 'w' && adj_dir == 'n') radians = -radians;
    //printf("Old:New = %d:%d\n", x1, x2);
    //printf("Rotating by %f radians...\n", radians);
    rotateZeroRadius(radians);
}
void perform_triangle_adjustment(int z, int i) {
    int adj_weight_prev;
    int adj_weight_next;
    int do_adj = 0;
    char adj_dir = '\0';
    ...
    if (do_adj) adjust_angle_triangle(adj_weight_prev, adj_weight_next, adj_dir);
}
...
void check_walls()
{
    ...
    if (checked_walls) perform_triangle_adjustment(z, i);
...
void follow_shortest(int do_adjustment) {
    ...
    while (short_path[k] != -1) {
        ...
        if (do_adjustment) {
            ...
            perform_triangle_adjustment(z, i);
...
/*
 * Function responsible for traversing the shortest path at the maximum possible speed
 */
int single_cell_time = (int) ((double) cell_size_ticks / 128.0 * 1000.0);
int single_rotation_time = (int) ((167.0 / DPT) / 128.0 * 1000.0);
int max_speed_ticks = 128;
int ir_wall_distance = 16;
void quick_rotate(char quick_dir) {
    if(direction == quick_dir) return;
    ...
    direction = quick_dir;
    //printf("Left speed: %d\n", max_speed_ticks * left);
    //printf("Right speed: %d\n", max_speed_ticks * right);
    drive_speed(max_speed_ticks * left, max_speed_ticks * right);
    //printf("Rotation time: %d\n", single_rotation_time);
    pause(single_rotation_time);
}
// Get the value of left IR sensor
int leftIR() {
  int irLeft = 0;
...
// Get the value of the right IR sensor
int rightIR() {
  int irRight = 0;
  for(int dacVal = 0; dacVal < 160; dacVal += 8)
    {                       
...
void quick_drive() {
    int i;
    int iterations = 10;
    int wait_time = single_cell_time / iterations;
    for(i = 0; i < iterations; i++) {
        int adj_left = min(0, leftIR() - ir_wall_distance) * 5;
        int adj_right = min(0, rightIR() - ir_wall_distance) * 5;
        drive_speed(max_speed_ticks, max_speed_ticks);
        pause(wait_time);
    }
}
void rush_shortest() {
    ...
    while (short_path[k] != -1) {
       // printf("Considering node #%d...\n", k);
        int current_cell = short_path[k++];
        if(current_cell == prev_cell) continue;

        if(cells[current_cell].x > cells[prev_cell].x) {
            //printf("Rotating east...\n");
            quick_rotate('e');
        }
        else if(cells[current_cell].x < cells[prev_cell].x) {
           // printf("Rotating west...\n");
            quick_rotate('w');
        }
        else if(cells[current_cell].y > cells[prev_cell].y) {
...
int main(void) {
    ...
    swap_direction('w');
    ping_wall(i);
    adjust_angle_triangle(prev_west_weight, west_weight, 's'); // Passing 's' instead of 'w' to ensure inverted angle
    ping_wall(i);
    adjust_one_wall(&west_weight);
    swap_direction('n');
